...
Click [**here**](../slides/module5_slides2.html) to view the accompanying **slides for Module 5, Part 2**.
...
Among many, the `survival` package is the most essential, which makes available the `Surv` class that is fundamental to a  survival analysis tools in R ecosystem. For more details, you may visit its [GitHub repository](https://github.com/therneau/survival) and read the package vignettes available on [the package CRAN page](https://cran.r-project.org/web/packages/survival/index.html)

`survminer` package helps visualization of survival models. Take a look at the [package website](http://www.sthda.com/english/rpkgs/survminer/) or the [GiHub repository](https://github.com/kassambara/survminer) for more information.
...
There is, of course, no shortage of third-party packages to offer additional functionalities relevant to time series analysis. Among many, `forcast` package seems to be a popular choice. Visit the [package website](http://pkg.robjhyndman.com/forecast/index.html)] to find out more. There also exists a full-fledged online textbook using `forcast` pacakge: Hyndman, R. & Athanasopoulos, G. (2018). [*Forecasting: Principles and Practice*](https://otexts.org/fpp2/).
...
### `caret` pacakage
`caret` is a shorthand for "Classification And REgression Training", and one of the most mature machine learning package out there. Check out the package vignettes on [the CRAN page](https://cran.r-project.org/web/packages/caret) and the package [GitHub repository](https://github.com/topepo/caret). There is also [an online textbook](http://topepo.github.io/caret/index.html) on using `caret` for machine learning.

### `mlr` pacakge
`mlr` is an abbreviation of "Machine Learning in R". Take a look at the vignettes on [the package CRAN page](https://cran.r-project.org/web/packages/mlr/) and its [GitHub repository](https://github.com/mlr-org/mlr/). Also visit [the package website]((https://mlr-org.github.io/mlr-tutorial/devel/html/)), which provide tutorials on the `mlr` API.
