...
```
* The first argument is a tabular data object
* `key` is an existing column containing the names of new columns, and `value` is another column containing the values for the new columns.
...
`unite()` and `sperate()` are great for cleaning up date or name variables.
